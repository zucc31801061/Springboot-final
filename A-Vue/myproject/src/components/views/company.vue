<template>
	<div class="company">
		<el-container>
			<el-header>
				<div class="welcome">你好，{{this.company.companyName}}</div>
				<div class="bottons">
					<el-col :span="12">
			...
			<el-container>
				<el-aside>
					<el-menu :default-active="this.$route.path" :default-openeds="['1']" class="el-menu-vertical-demo"
						router>
						<el-submenu index="1">
							<template slot="title">
							...
							<el-menu-item-group>
								<el-menu-item :index="myempl">我的员工</el-menu-item>
								<el-menu-item :index="otherempl">员工招聘</el-menu-item>
							</el-menu-item-group>
						</el-submenu>
...
<script>
	export default {
		...
		methods: {
			...
			update(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						if (this.pwdform.oldPwd == this.company.companyPwd) {
							this.axios.get("http://localhost:8888/A-Demo/company/updatePwd?id=" + this.$route
								.params
								.id + "&pwd=" + this.pwdform.newPwd).then(
								res => {
									this.$message({
										showClose: true,
										type: 'success',
								...
								this.axios.get("http://localhost:8888/A-Demo/company/select?id=" + this.$route.params.id)
									.then(
										res => {
		...
		mounted() {
			this.myempl = '/company/' + this.$route.params.id + '/myempl';
			this.otherempl = '/company/' + this.$route.params.id + '/otherempl';
			this.myinfo = '/company/' + this.$route.params.id + '/myinfo';
		},
		created() {
